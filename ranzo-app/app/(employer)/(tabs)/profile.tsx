...
/** M-X01: Employer profile */
export default function EmployerProfileTab() {
  const { t } = useTranslation();
  const router = useRouter();
  const signOut = useAuthStore((s) => s.signOut);
  const company = useEmployerWizardStore((s) => s.draft.companyName);

  const MENU = [
    { labelKey: 'menus.editCompanyProfile', path: '/onboarding/employer/step-1' },
    { labelKey: 'menus.subscription', path: '/(employer)/subscription' },
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={Typography.h1}>{company || t('menus.employer')}</Text>
        <Text style={styles.badge}>{t('menus.hireTalent')}</Text>
        {MENU.map((m) => (
